...
class NewsAPIs:
    ...
    def get_local_news():
        """
        Busca notícias do banco SQLite local das últimas 3 horas
        """
        ...
        try:
            # Filtro temporal - últimas 3 horas
            ponto_de_corte = datetime.now() - timedelta(hours=3)
            ...
            logging.info(f"[FILTRO] SQLite Local: {len(articles)} artigos encontrados (últimas 3h).")
